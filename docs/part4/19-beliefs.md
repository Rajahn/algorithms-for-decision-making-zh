...
- **核心心智模型（大雾弥漫中的航海罗盘）**：在真实世界中，传感器永远带有毛刺和盲区（POMDP）。你永远无法 100% 确认自己身处哪个确切岛屿，你手里只有一张概率地图——**置信状态（Belief State） $\mathbf{b}$**。
- **充分统计量的物理意义**：只要你维护好当前的置信分布，过去十年你经历过的全部航海日记（历史观测和动作）就可以随手烧毁——所有的因果经验都已经被无损压缩在当前的置信分布之中！
- **三大滤波算子对照**：
  1. **离散贝叶斯滤波**：在概率单纯形上逐个做乘法和重新归一化；
  2. **卡尔曼滤波（高斯优雅）**：线性世界里高斯的后验依然是高斯，卡尔曼增益负责在“模型预测推演”和“传感器测量矫正”之间按方差大小称重；
  3. **粒子滤波（蒙特卡洛云团）**：在非线性非高斯山谷中，撒一把成百上千个代表物理位置的带权小粒子，通过“运动扩散 $\to$ 观测重加权 $\to$ 适者生存重采样”追踪任意怪异形态的概率云。
:::
---
