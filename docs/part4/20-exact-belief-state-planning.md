...
- **核心心智模型（置信空间的锦囊妙计）**：虽然物理状态只有有限个，但置信状态是连续无穷的（单纯形空间）。索恩迪克定理证明：POMDP 的最优价值函数恒为**分段线性凸函数（PWLC）**！
  - 每一个锦囊妙计（条件计划树，如“如果听到哭声就喂奶，没听到就逗乐”）在数学上对应置信空间中的一架倾斜超平面——**Alpha 向量 $\boldsymbol{\alpha}$**；
  - 智能体的最优价值，就是所有这些超平面的**最高天花板上包络面（$\max_{\boldsymbol{\alpha}} \boldsymbol{\alpha}^\top \mathbf{b}$）**；
  - **凸性的深刻直觉**：处于单纯形顶点（完全确定物理状态）时的价值永远高于单纯形中心（极度混沌迷茫）时的价值，数学证明了“确凿的情报本身具有不可剥夺的正价值”！
- **剪枝的本质**：绝大多数条件计划超平面在几何上被其他更优秀的锦囊完全压在底下，用线性规划可以把这些无用的被支配超平面彻底扫地出门。
:::
---
