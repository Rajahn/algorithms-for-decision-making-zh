...
- **核心心智模型（走一步看三步）**：离线规划是“把天下所有可能发生的情况全背下来，随身带一本大百科全书”；在线规划是“出门不带书，走到哪一步，就在眼前这个局部状态往前推演模拟几步，当场拍板，走完第一步把推演树扔掉重新算”。
- **两大工程顶梁柱**：
  - **稀疏采样（Sparse Sampling）**：为什么高维随机环境能在线算？因为大数定律保证了**为了测准均值，每次抽样的分支数量 $m$ 仅取决于需要的精度，而跟物理世界总共有多少亿个状态完全无关**！
  - **蒙特卡洛树搜索（MCTS）**：AlphaGo 与现代博弈 AI 的灵魂。通过“选择（UCB1 兼顾高分与冷门） $\to$ 扩展（生出新叶子） $\to$ 模拟（蒙特卡洛快速推演到底） $\to$ 反向传播”，将珍贵的算力非对称地狂砸在最具潜力的优质着法上。
- **模型预测控制（MPC）的防翻车哲学**：哪怕优化器算出了未来 20 步的完美航迹，智能体**也永远只坚决执行第 1 步控制信号**，然后立即用传感器重新测量真实世界并重新规划，天然免疫累积模型漂移。
:::
---
