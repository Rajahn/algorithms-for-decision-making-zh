...
- **核心心智模型（巧夺天工的似然比技巧）**：在未知物理规律的环境中，你怎么可能对期望回报求参数导数？微积分的对数求导恒等式 $\nabla p = p \nabla \log p$ 施展了魔法：
  $$
  \nabla_{\boldsymbol{\theta}} \mathbb{E}[R] = \mathbb{E}\left[ (\nabla_{\boldsymbol{\theta}} \log \pi_{\boldsymbol{\theta}}(a \mid s)) \cdot R \right]
  $$
  **最惊人的数学奇迹在于：环境未知的物理转移概率在求导后彻底蒸发消失了！** 智能体只要知道自己打出动作的概率梯度，用最终拿到的实际回报乘上去，就得到了全无偏的策略梯度！
- **降方差两大斧**：
  1. **因果性（Reward-to-Go）**：未来的动作无法改变过去，果断把动作发生之前的历史奖励全部砍掉；
  2. **状态基线（Baseline）**：在总回报中减去当前状态的平均期望底气 $V(s)$。做得比平时好就增加概率，比平时差就压低概率，数学期望严格不变（零系统偏差），但采样抖动方差大幅暴跌。
:::
---
