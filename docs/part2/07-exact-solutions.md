...
- **核心心智模型（长线复利与倒着走的智慧）**：单步决策只看眼前的即时肉包子，而序贯决策（MDP）关注长达数十步乃至无穷远的终身累积贴现收益。贝尔曼方程的本质是**“跨期等价兑换”**——今天这个状态的综合价值，等于**我今天当下能拿到的即时奖励，加上把明天的未来期望价值打个折折算回今天**。
- **动态规划两大神兽**：
  - **策略迭代（Policy Iteration）**：两阶段交替。先闭关把当前老策略彻底算透（解一组线性方程组），然后看看每个状态能不能换个更好的动作；由于策略总数有限且每次都单调变得更好，有限轮内必收敛至全局最优！
  - **价值迭代（Value Iteration）**：不显式维护策略，直接用贝尔曼最优算子持续刷洗全图价值。巴拿赫不动点定理保证了算子每刷一次，全图与真值的最大误差至少缩水到原来的 $\gamma$ 倍（严格收缩映射），无脑迭代必定收敛。
- **状态价值 $V(s)$ 与动作价值 $Q(s, a)$ 区别**：$V(s)$ 是指你站在这块地盘上的平均底气；$Q(s, a)$ 是指你在这块地盘上如果特定打出这一拳、随后回归正常水平的期望总收益。
:::
---
