...
- **核心心智模型（黑盒盲拧螺丝）**：前面几章都在苦苦估计“价值曲面”，但智能体的终极目标是“策略行为”。策略搜索干脆跳过价值函数：直接给策略装上参数 $\boldsymbol{\theta}$（如神经网络旋钮），把环境当成一个大黑盒，通过观察旋钮调整后的整体推演得分来寻优。
- **探索策略谱系**：
  - **Hooke-Jeeves 模式搜索**：每次顺着一个坐标轴试探一步，有效就前进，撞墙就缩小步长；
  - **遗传算法（GA）**：养一群参数，适者生存，让高分个体杂交交配并施加随机突变；
  - **交叉熵方法（CEM）**：用高斯分布漫天撒网，挑出前 20% 的精英个体，重新把高斯椭球收缩覆盖这批精英；
  - **自然进化策略（NES）与镜像采样**：在当前旋钮周围加对称白噪声（$\pm \sigma \boldsymbol{\epsilon}$），用成对的性能差直接拼出参数梯度的无偏近似，无需反向传播也能攀登高维山峰。
:::
---
