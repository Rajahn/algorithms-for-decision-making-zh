...
- **核心心智模型（系上安全带走悬崖）**：直接拿算出来的策略梯度做一阶梯度上升（$\theta \leftarrow \theta + \alpha g$），在强化学习里极容易暴毙——因为参数空间里迈出的一小步，可能导致概率分布发生雪崩式突变，智能体一下子跌入从未见过的死胡同，策略彻底崩溃瘫痪。
- **信任域与 PPO 的防暴毙设计**：
  - **自然策略梯度（NPG）**：不在欧氏空间量距离，而在概率流形上用 KL 散度（费雪矩阵）量距离，具有参数标度不变性；
  - **TRPO**：严格加上 KL 散度硬约束 $\le \delta$，步步为营单调递增，但要解昂贵的二阶共轭梯度；
  - **PPO（平民神级算法）**：放弃繁琐的二阶计算，直接设计了一个**“悲观裁剪下确界”**。当动作改得太猛（概率比率超过 $1 \pm \epsilon$ 时），目标函数直接截断平躺、梯度清零，用最纯粹的一阶优化实现了堪比二阶信任域的超强稳定性！
:::
---
