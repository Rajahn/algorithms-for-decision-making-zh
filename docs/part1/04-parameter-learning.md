...
- **核心心智模型（经验与先验的拉锯战）**：参数学习就是从历史观测数据中反推概率模型的参数 $\boldsymbol{\theta}$。极大似然估计（MLE）是绝对的“唯经验论者”（数数频数直接做除法）；贝叶斯学习是“带着先验常识的经验论者”（把先验理解为虚拟实验次数，如拉普拉斯平滑）。
- **共轭先验的代数美感**：为什么教科书偏爱 Beta 和 Dirichlet 分布？因为它们与二项分布和多项分布是“天作之合”——**先验分布与观测后验分布具有完全相同的代数形式**，更新后验仅仅相当于把物理观测计数值与先验伪计数直接相加！
- **缺失数据与 EM 算法直觉**：当数据存在字段残缺时，无法直接算极大似然。**EM 算法本质是“猜忌-验证”双人舞**：
  - **E 步（根据当前参数猜隐变量分布）**：“假设当前参数是对的，把缺失数据按概率期望填补完整”；
  - **M 步（根据填补数据更新参数）**：“假设填补的数据是真的，重新做一次简单的极大似然”；
  - 循环交替，保证每一次对数边际似然都单调递增，平稳滑向局部最优。
:::
---
