...
- **核心心智模型（给数据画因果拓扑图）**：不仅参数是未知的，变量之间谁是谁的原因（连线与箭头）同样未知。结构学习就是在大海捞针般的 DAG 拓扑空间中，寻找最能解释数据的因果图。
- **为什么不能直接用极大似然挑图？**：向网络中多加一条边，模型的拟合自由度必然增加或不变，似然值只增不减！如果只看似然，算法必然输出一个所有节点两两互连的**全连接图**（严重过拟合，毫无因果解释力）。
- **奥卡姆剃刀的数学实现**：
  - **贝叶斯评分（Bayesian Score）**：对所有可能参数积分，高维复杂模型因为参数空间太大，其后验积分会自动受到几何惩罚；
  - **BIC / MDL 准则**：公式 $\text{BIC} = \text{似然度} - \frac{\text{参数量}}{2} \log m$，右边这项正是对模型参数规模的显式惩罚！
- **马尔可夫等价类与本质图**：纯靠静态观测数据，**无法区分骨架相同且非道德 v-结构相同的 DAG**（例如 $A \to B \to C$ 与 $A \leftarrow B \to C$ 具有完全一样的统计独立性），数据只能辨识到部分有向图（PDAG）等价类级别。
:::
---
