...
- **核心心智模型（从算命到拍板）**：前面 5 章都是在算概率（算命），本章开始做出行动（拍板）。冯·诺依曼证明：只要一个人的偏好满足 6 条基本理性公理，就必然存在一个标量效用函数 $U(s)$，使得其所有理性抉择完全等价于追求**期望效用最大化（MEU）**。
- **风险态度的几何本质**：效用函数对金钱的凹凸性决定了人的胆量：
  - **严格凹函数（边际效用递减）** $\implies$ **风险厌恶**（宁愿少赚一点也要确定性，这是保险行业成立的数学根基）；
  - **严格凸函数** $\implies$ **风险喜好**；**线性函数** $\implies$ **风险中性**。
- **信息价值（VOI）的非负性物理意义**：在做重大决策前，花钱买情报到底值不值？
  $$
  \text{VOI} = \mathbb{E}[\text{得知情报后的最优期望效用}] - \text{盲猜时的最优期望效用}
  $$
  因为情报到手后你永远可以选择“坚持原判”，新信息只给决策者赋予了更灵活的应变权，因此**理性的信息价值恒大于等于零（$\text{VOI} \ge 0$）**。
:::
---
