...
- **核心心智模型（放下执念，直接记账）**：无模型方法彻底放弃给世界建物理沙盘的幻想（因为现实世界太复杂了，风阻、摩擦、形变根本建不准）。它像一个老练的记账会计：每在一个状态 $s$ 迈出一步 $a$，拿到真金白银的奖励 $r$，走到新状态 $s'$，就直接用增量更新公式给账本上的 $Q(s, a)$ 账目做一次修正。
- **Q-Learning vs SARSA 的性格差异**：
  - **Q-Learning（离策略，无畏赌徒）**：更新公式取的是 $\max_{a'} Q(s', a')$。它坚信自己未来必然会选择最完美的一招，因此在悬崖边行走时敢于紧贴悬崖走直线；
  - **SARSA（在策略，谨慎现实主义）**：更新公式取的是自己实际摇号抽出的下一个动作 $Q(s', a')$。它清醒地意识到自己未来有 $\epsilon$ 的概率会打滑，因此它会主动选择绕着悬崖走大弧线，以求稳妥。
- **奖励塑形（Reward Shaping）的势能不变量**：为了加速学习，能不能给稀疏奖励加点人工小甜头？能，但必须遵循 $\gamma \Phi(s') - \Phi(s)$ 的势能差形式，通过裂项级数相消确保最优策略不变，否则智能体必然学会刷分作弊！
:::
---
...
## 17.5 奖励塑形与势能理论 (Reward Shaping)

在很多现实复杂控制与多阶段规划任务中，环境本身的真实物理奖励极其稀疏（例如机械臂装配零件只有在最终卡榫成功啮合的瞬间才能获得一次 $+1$ 奖励）。在如此广袤的零奖励荒漠中，盲目的随机试探可能经历数百万步也无法获得一次正反馈。

**奖励塑形（Reward Shaping）**旨在通过向环境添加人工构造的**附加引导奖励（Auxiliary Reward）** $F(s, a, s')$，在每一个单步为智能体提供导航线索，使其实际接收到的塑形奖励为：
$$
\tilde{R}(s, a, s') = R(s, a, s') + F(s, a, s')
$$

::: danger 常见陷阱：奖励作弊（Reward Hacking）
如果工程师随意设计辅助奖励（例如每当前进一步就奖励 $+0.1$），智能体往往会学会**非预期的投机取巧恶劣行为**——例如在原地反复来回打转或绕大圈刷取正奖励，而永远不去完成最终的真正终点目标！
:::

### 基于势能的奖励塑形定理 (Potential-Based Reward Shaping, Ng, Harada & Russell, 1999)
吴恩达（Andrew Ng）等人证明了强化学习领域最重要的理论定理之一：
为使辅助奖励函数 $F$ 在**任意底层转移模型下，严格保证最优策略与原环境完全同一（Policy Invariance）**，辅助奖励函数**必须且只能表示为某个实值势能状态函数 $\Phi(s)$ 的贴现差分形式**：
$$
F(s, a, s') = \gamma \Phi(s') - \Phi(s)
$$
式中 $\Phi: \mathcal{S} \to \mathbb{R}$ 为任意赋予状态的势能高低估值。

**数学证明直觉**：
将基于势能的奖励代入任意有限轨迹的累积贴现求和中：
$$
\sum_{t=0}^\infty \gamma^t \tilde{R}(s_t, a_t, s_{t+1}) = \sum_{t=0}^\infty \gamma^t R(s_t, a_t, s_{t+1}) + \sum_{t=0}^\infty \gamma^t (\gamma \Phi(s_{t+1}) - \Phi(s_t))
$$
考察后半部分的势能差分求和：
$$
(\gamma \Phi(s_1) - \Phi(s_0)) + \gamma (\gamma \Phi(s_2) - \Phi(s_1)) + \gamma^2 (\gamma \Phi(s_3) - \Phi(s_2)) + \dots
$$
所有中间时刻的势能项严格形成**裂项级数相消（Telescoping Sum）**，最终整个无穷级数仅留下起始状态的常数 $-\Phi(s_0)$！
因此，任何轨迹在势能塑形下的新总回报，恒等于原总回报减去初始状态常数 $-\Phi(s_0)$，这完全等价于效用函数的正仿射平移变换，**从而在数学上严格保证了所有动作相对优劣偏好完全不变，彻底杜绝了智能体刷分作弊的风险**！

---

## 17.6 线性函数逼近 Q-Learning (Linear Function Approximation)
...
## 17.8 课后习题与官方详细解答 (Exercises & Solutions)
