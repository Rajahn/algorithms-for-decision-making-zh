...
- **核心心智模型（建立沙盘并在心里推演）**：基于模型的强化学习（MBRL）绝不盲目试错，而是像军事参谋一样，在交互过程中用心绘制一张包含物理转移规律 $T$ 和奖励 $R$ 的“世界沙盘”。一旦沙盘建立完成，智能体就可以在自己的大脑模拟器中反复推演，极度节约物理世界的真实尝试成本。
- **R-MAX 的主动探索魔法**：为了防止因为模型估计不全而早熟锁死在平庸方案中，R-MAX 做出了绝妙的规定：**任何访问次数少于 $m$ 次的未知状态，在沙盘中一律被强制定为具有宇宙最高奖励 $R_{\max}$！**
  - 这在沙盘中凭空制造了一个巨大的高收益引力源；
  - 动态规划器为了追求高分，会自动规划出一条引导智能体穿越已知领地、直扑未知边界的最优路径；
  - 智能体顺理成章地完成了对未知世界的主动探索与充分认知。
:::
---
